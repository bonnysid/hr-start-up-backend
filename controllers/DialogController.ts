...
class DialogController {
  ...
  async getDialogInfo(req: Request, res: Response) {
    try {
      ...
      const dialog = await Dialog.findOne({ users: user.id, _id: dialogId }).populate<any>([{
        path: 'messages',
        populate: {
          path: 'user'
        }
      ...
      await Promise.all(dialog.messages.filter((it: any) => !it.read && it.user._id.toString() !== user.id).map(async (it: any) => {
        it.read = true;
        await it.save();
      }))
  ...
  async getDialogs(req: Request, res: Response) {
    try {
      const user = (req as any).user;
      const dialogs = await Dialog.find({ users: user.id }).populate([{
        path: 'messages',
        populate: {
          path: 'user'
        }
